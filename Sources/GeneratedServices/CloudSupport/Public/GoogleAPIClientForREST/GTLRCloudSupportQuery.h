 ...
 *  separated by `AND` and specified with `=`: - `state`: Can be `OPEN` or
 *  `CLOSED`. - `priority`: Can be `P0`, `P1`, `P2`, `P3`, or `P4`. You can
 ...
 *  case creator. To search across `displayName`, `description`, and comments,
 ...
 *  `update_time` only supports the greater than operator (`>`). If you are
 *  using the `v2` version of the API, you must specify the case parent in the
 *  `parent` field. If you provide an empty `query`, all cases under the parent
 *  resource will be returned. If you are using the `v2beta` version of the API,
 *  you must specify the case parent in the `query` field using one of the two
 *  fields below, which are only available for `v2beta`. The `parent` field will
 *  be ignored. - `organization`: An organization name in the form
