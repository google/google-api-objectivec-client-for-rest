...
/**
 *  The INACTIVE state means that the Channel cannot receive events permanently.
 *  There are two possible cases this state can happen: 1. The SaaS provider
 *  disconnected from this Channel. 2. The Channel activation token has expired
 *  but the SaaS provider wasn't connected. To re-establish a Connection with a
 *  provider, the subscriber should create a new Channel and give it to the
 *  provider.
 *
 *  Value: "INACTIVE"
 */
 ...
 *    @arg @c kGTLREventarc_Channel_State_Inactive The INACTIVE state means that
 *        the Channel cannot receive events permanently. There are two possible
 *        cases this state can happen: 1. The SaaS provider disconnected from
 *        this Channel. 2. The Channel activation token has expired but the SaaS
 *        provider wasn't connected. To re-establish a Connection with a
 *        provider, the subscriber should create a new Channel and give it to
 *        the provider. (Value: "INACTIVE")
