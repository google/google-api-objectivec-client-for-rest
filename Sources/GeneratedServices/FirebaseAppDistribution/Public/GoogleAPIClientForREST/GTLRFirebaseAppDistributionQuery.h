 ...
 *  Supported fields: - Time fields supporting `<`, `<=`, `>` and `>=`;
 *  expecting an RFC-3339 formatted string: - `create_time` (or `createTime`) -
 *  `update_time` (or `updateTime`) - `expire_time` (or `expireTime`) - Text
 *  fields supporting `=`. The compared text can contain a wildcard character
 *  (`*`) at the beginning and/or end of the string which also enables
 *  case-insensitive matching: - `release_notes.text` (or `releaseNotes.text`) -
 *  `display_version` (or `displayVersion`) - `build_version` (or
 *  `buildVersion`). Examples: - `createTime <= "2021-09-08T00:00:00+04:00"` -
 *  `expire_time > "2021-09-08T00:00:00+04:00"` - `releaseNotes.text="fixes" AND
